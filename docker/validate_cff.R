...
# clean files
if (file.exists("citation_cff.md")) {
  unlink("citation_cff.md")
}
if (file.exists("issue.md")) {
  unlink("issue.md")
}
...
cat("Validating cff\n")
citfile <- cffr::cff_read(cffpath)

# Convert to list
citfile <- as.list(citfile)
...
if (result == FALSE) {
  ...
  write(
    knitr::kable(get_errors[, c("field", "message")], align = "l"),
